...
- 当前 `docs/requirements/<开发者账号>/<REQ>/requirement.md` 与 `ai-task-scope.yaml`：单任务权限和验收。
...
- `settings/process-configuration`：字段、分区、交付件、状态规则、布局、模板版本和配置升级。其目录定义字段/交付件的代码基线；业务模块仅经公开入口读取，配置登记不取代页面 JSX 接入。
