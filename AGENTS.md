...
项目负责人可在当前对话中明确授权 AI 启动 RADAR 的本地开发服务，并由应用按既有配置访问 `data/**`。此例外仅覆盖服务启动、健康检查和经应用界面执行的正常本地调试流程；当项目负责人明确要求将本地配置固化为初始种子时，可只读查询阶段分区、输入项、交付件及其规则等配置表。不得读取用户、业务记录、附件内容或上传模板二进制；不得导出、复制、展示或向外发送本地数据，也不允许连接生产环境。任务范围文件必须同步登记该例外后方可执行。
