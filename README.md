...
3. 启动后端生产模式。
4. 访问根路径确认静态资源可加载。
5. 登录后检查 `/api/health`、菜单权限、投产点加载和列表接口。
...
复杂函数、业务规则、迁移脚本和导入导出逻辑需要中文注释说明意图。

### 后端约定

- 简单配置表优先使用 [server/src/lib/crud.js](server/src/lib/crud.js)。
- 列表查询优先使用 [server/src/lib/query.js](server/src/lib/query.js)，并配置列白名单。
- 写操作必须考虑过程留痕。
- 编号生成必须走 [server/src/lib/code-gen.js](server/src/lib/code-gen.js)。
- 状态终态判断必须走 [server/src/lib/status.js](server/src/lib/status.js)。
- 必填项校验必须走 [server/src/lib/required-fields.js](server/src/lib/required-fields.js)。
- 需求和工单统一读取时优先使用 [server/src/lib/work-items.js](server/src/lib/work-items.js)。
- SQL 不应直接拼接用户输入。列表筛选、排序字段必须使用白名单。
- 新增模块时需要同步权限目录、菜单、路由和必要的导入导出能力。

### 前端约定

- 页面权限由菜单配置和 `<Can>` 共同控制。
- 列表优先复用 `DataTable`、`FilterPanel`、`ImportModal`。
- 状态展示优先使用 `StatusBadge`。
- 详情编辑器优先复用 `EditorShell`、`HistoryDrawer`、`AttachmentField`。
- 人员选择优先使用 `PersonPicker`。
- 系统选择优先使用 `SystemSelect` 或模块内统一选择器。
- 投产点显示优先使用 `ReleasePointText`。
- 主题色、明暗模式和响应式布局应遵循现有样式变量。

### 数据库约定

- SQLite 迁移必须幂等或由 `_migrations` 控制只执行一次。
- 新增字段后需要同步 TDSQL 初始化脚本。
- 新增 JSON 字段后需要同步迁移脚本中的 JSON 字段集合。
- 新增业务表后需要同步迁移工具的表顺序。
- 新增权限模块后需要同步种子权限和权限目录。

### Git 与交付约定

- 提交前至少运行后端测试和前端构建。
- 文档中的模块、环境变量、脚本命令应与代码保持一致。
- 不提交本地数据库、附件、构建产物、日志和本地工具配置。
...
- [PRODUCT.md](PRODUCT.md)：产品定位、用户角色、品牌人格、设计原则。
- [DESIGN.md](DESIGN.md)：颜色、字体、圆角、组件和视觉规范。
- [AI-GUIDE.md](AI-GUIDE.md)：项目协作说明。
- [MIGRATION.md](MIGRATION.md)：迁移相关补充说明。
- [DASHBOARD-PLAN.md](DASHBOARD-PLAN.md)：仪表盘规划记录。
