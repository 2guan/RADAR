...
> **R**equirement & **A**gile **D**evelopment **A**dministration **R**esource
>
...
> 密码可由 `ADMIN_PASSWORD` 环境变量覆盖（`config.js` 默认 `Admin@2026`，`docker-compose.yml` 与本仓库设为 `admin2026`）。生产环境务必修改。
...
| `JWT_SECRET` | `radar-dev-secret-...` | **生产务必改为随机长字符串** |
...
| `ADMIN_PHONE` / `ADMIN_PASSWORD` | `admin` / `Admin@2026` | 初始超级管理员 |
| `PAMS_BASE_URL` / `PAMS_API_KEY` / `PAMS_TIMEOUT` | — | 外部 PAMS 问题管理系统同步源 |

附件扩展名白名单：`pdf doc docx xls xlsx ppt pptx txt md csv zip rar 7z png jpg jpeg gif bmp svg`。
