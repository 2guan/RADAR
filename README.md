...
| 机构负责人 | 投产评审会签角色之一 |
| 需求管理、架构管理、测试管理、项目管理 | 投产评审会签角色之一 |
| 安全管理、质量管理、运维负责人、配置管理员 | 投产评审会签角色之一 |
...
- 首次打开详情时自动创建投产任务和 9 个会签项。
- 支持维护投产负责人、投产状态、评审状态。
- 会签角色：机构负责人、需求管理、架构管理、测试管理、项目管理、安全管理、质量管理、运维负责人、配置管理员。
- 会签结果：未签署、已签署、已驳回。
- 非超级管理员只能签署自己拥有的会签角色。
...
| `0022_release_task_apply_point.sql` | 投产任务申请投产点 |
| `0023_impact_coverage.sql` | 影响性分析与测试覆盖分析 |
| `0024_role_signoff_check_content.sql` | 会签检查内容配置 |
| `0025_merge_intake_permissions.sql` | 承接权限合并 |
| `0026_add_status_edit_permission.sql` | 状态编辑权限 |
| `0027_split_test_permissions.sql` | 按测试类型拆分权限 |
| `0028_unify_release_process_status.sql` | 投产流程状态统一 |
...
| `overview` | 版本概览 | 查看 |
| `requirement` | 需求分析 | 查看、新增、编辑、调整状态、删除、导入、导出 |
| `ticket` | 工单分析 | 查看、新增、编辑、调整状态、删除、导入、导出 |
| `issue` | 问题管理 | 查看、同步、清空 |
| `dev` | 开发管理 | 查看、新增、编辑、调整状态、删除、导入、导出 |
| `test.SIT` / `test.UAT` / `test.NFT` / `test.SEC` | 四类测试管理 | 各自具备查看、新增、编辑、调整状态、删除、导入、导出 |
| `release_apply` | 投产申请 | 查看、新增、编辑、删除、导入、导出 |
| `release` | 投产审批 | 查看、编辑、调整状态、评审会签、投产登记、导出 |
...
如果镜像已在其他构建环境制作并推送到镜像仓库，可在 `.env` 中把 `RADAR_IMAGE` 改为远端镜像地址，然后在服务器执行：
...
- [AI-GUIDE.md](AI-GUIDE.md)：面向开发者与开发助手的项目上下文、定位方式和改动检查单。
- [COLLABORATION.md](COLLABORATION.md)：面向多人并行开发的职责域、公共服务、迁移、集成和验收规范。
- [PRODUCT.md](PRODUCT.md)：产品边界、领域对象、流程和验收口径。
- [DESIGN.md](DESIGN.md)：当前主题、组件、响应式和无障碍约束。
- [MIGRATION.md](MIGRATION.md)：SQLite 与 TDSQL 的迁移、备份和恢复流程。
