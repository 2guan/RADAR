/**
 * @file page.tsx
 * @description PAMS 登录技术人员/管理员个人名下所负责的问题页面
 * @author hengguan
 * @date 2026-05-20
 */
...
import { App, Table, Button, Space, Tag, Input, Select, message, Drawer, Form, Row, Col, Typography, Modal, List, Card, Grid, Badge, Tooltip, DatePicker } from 'antd';
...
export default function MyIssuesPage() {
    ...
    const fetchIssues = async () => {
        // Support both authenticated users and guests using phone number
        const contact = user?.contact || (typeof window !== 'undefined' ? (localStorage.getItem('guest_tracker_contact') || localStorage.getItem('guest_submitter_contact')) : null);
        if (!contact) {
            setLoading(false);
            return;
        }
    ...
    const handleUserSelect = (value: string, option: any) => {
        form.setFieldsValue({
            ...
            handler_org: option.org,
            handler_contact: option.contact
        });
    };
    ...
    const handleRowClick = async (record: Issue) => {
        try {
            ...
            if (data.success) {
                ...
                const initialValues = {
                    ...fullIssue,
                    handler_name: user?.real_name || fullIssue.handler_name,
                    handler_org: user?.organization || fullIssue.handler_org,
                    handler_contact: user?.contact || fullIssue.handler_contact
                };
    ...
    const handleSystemChange = (value: string) => {
        const system = dicts.system.find(d => d.item_key === value);
        if (system && system.description) {
            try {
                const desc = JSON.parse(system.description);
                form.setFieldsValue({
                    module: desc.module,
                    business_group: desc.bg
                });
            } catch (e) {
                // Ignore
            }
    ...
    useEffect(() => {
        if (!user && typeof window !== 'undefined') {
            setGuestContactInput(localStorage.getItem('guest_tracker_contact') || '');
        }
    }, [user]);

    const handleGuestContactSearch = () => {
        if (!guestContactInput.trim()) {
            message.warning('请输入手机号');
            return;
        }
        localStorage.setItem('guest_tracker_contact', guestContactInput.trim());
        setPage(1);
        fetchIssues();
    };
    ...
    return (
        <div>
            ...
            {/* Action Buttons & Guest Search */}
            <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
                <div>
                    {!user && (
                        <Space>
                            <Input
                                placeholder="输入手机号查询我的问题"
                                value={guestContactInput}
                                onChange={(e) => setGuestContactInput(e.target.value)}
                                style={{ width: 220 }}
                                prefix={<UserOutlined />}
                                onPressEnter={handleGuestContactSearch}
                            />
                            <Button type="primary" onClick={handleGuestContactSearch}>
                                查询
                            </Button>
                        </Space>
                    )}
                ...
                screens.xs || (screens.sm && !screens.md) ? (
                    <List
                        ...
                        renderItem={item => {
                            ...
                            return (
                                <List.Item onClick={() => handleRowClick(item)} style={{ cursor: 'pointer', padding: '0', marginBottom: 8 }}>
                                    <Badge.Ribbon
                                        text={item.is_major ? "重大" : null}
                                        color="red"
                                        style={{ display: item.is_major ? 'block' : 'none' }}
                                    >
                                        <Card
                                            size="small"
                                            title={
            ...
            <Drawer
                ...
                open={drawerVisible}
                styles={{ body: { padding: 0 }, wrapper: { width: screens.xs ? '100%' : 800 } }}
            >
                {currentIssue && (
                    <IssueDetailView
