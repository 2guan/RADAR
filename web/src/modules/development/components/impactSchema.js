/**
 * 文件：web/src/modules/development/components/impactSchema.js
...
// 字段定义：key -> { label, type, min, max, col, rows, requiredWhen }
...
export function validateItem(it) {
  ...
  for (const key of fields) {
    ...
    if (def.type === 'systems') {
      ...
      if ((def.required || def.requiredWhen) && arr.length === 0) errs.push(`「${def.label}」至少填写一个系统`);
      continue;
    }
