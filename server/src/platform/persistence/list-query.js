/**
 * 文件：server/src/platform/persistence/list-query.js
...
import { all, get } from './engine/index.js';
 ...
 * @param {Function} [opts.extensionFilterBuilder] 扩展字段筛选构造器，由阶段配置模块公开提供
...
export async function listQuery(opts) {
  ...
  for (const f of filters) {
    ...
    // 扩展字段筛选由阶段配置模块通过公开回调构建；平台层不感知业务字段表。
    if (extensionScopeKey && extensionEntityType && String(field).startsWith('extension:')) {
      ...
      const extensionFilter = await extensionFilterBuilder?.({
        table, scopeKey: extensionScopeKey, entityType: extensionEntityType, fieldKey, op, value,
      });
      if (extensionFilter?.where) {
        where.push(extensionFilter.where);
        params.push(...(extensionFilter.params || []));
      }
      continue;
    }
