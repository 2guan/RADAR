...
import { windowIds, inClause, getDictDisplayMap, resolveDictAttr, resolveExistingDictAttr, resolveOrganizationValues, resolveSystemCodes } from '../settings/reference-data/index.js';
  ...
  fastify.post('/tickets/list', { preHandler: fastify.requirePerm('ticket', 'view') }, async (request) => {
    ...
    for (const f of filters) {
      ...
      if (f.field === 'content') {
        ...
        if (codes.length) {
          const placeholders = codes.map(() => '?').join(',');
          wh.push(dialect.jsonArrayOverlaps(`ticket.${f.field}`, placeholders));
          params.push(...codes);
        }
        ...
        if (people.length) {
          ...
          wh.push(dialect.jsonArrayOverlaps('ticket.proposer', placeholders));
          params.push(...people);
        }
      } else {
