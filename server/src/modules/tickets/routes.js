...
const COLUMNS = [
  'id', 'ticket_code', 'title', 'summary', 'status', 'priority', 'ticket_type', 'propose_dept', 'proposer',
...
const WRITABLE = [
  'ticket_code', 'title', 'summary', 'status', 'priority', 'ticket_type', 'propose_dept', 'proposer', 'yn_owner', 'jk_owner',
  'propose_time', 'main_systems', 'collab_dev_systems', 'collab_test_systems', 'release_point_id',
  'issue_no', 'is_accounting',
];
const LABELS = {
  ticket_code: '工单编号', title: '工单概述', summary: '工单详情', status: '工单状态', priority: '优先级', ticket_type: '工单类型',
  is_accounting: '是否涉账',
...
export default async function ticketRoutes(fastify) {
  ...
  fastify.post('/tickets/list', { preHandler: fastify.requirePerm('ticket', 'view') }, async (request) => {
    ...
    result.list = result.list.map((row) => {
      ...
      decoded.task_status = taskStatuses[decoded.ticket_code]?.display || '需求/工单分析-未开始';
      decoded.task_status_short = taskStatuses[decoded.ticket_code]?.shortDisplay || '工单 · 未开始';
      decoded.task_status_value = taskStatuses[decoded.ticket_code]?.status || '未开始';
        ...
        try {
          ...
          if (exists) {
            ...
            if (changes.length > 0) {
              await run(
                `UPDATE ticket SET 
                   title=?, summary=?, status=?, priority=?, ticket_type=?, is_accounting=?, propose_dept=?, proposer=?, yn_owner=?, jk_owner=?,
              ...
              await auditUpdate('ticket', exists.id, code, request.currentUser?.name, exists, {
                title: r.title, summary: r.summary || null, status, priority, ticket_type: reqType || null, is_accounting: isAccounting,
            ...
            const res = await run(
              `INSERT INTO ticket 
                 (ticket_code, title, summary, status, priority, ticket_type, is_accounting, propose_dept, proposer, yn_owner, jk_owner,
