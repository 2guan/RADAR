...
const SIGNOFF_CHECK_CONTENT = {
  安全负责人: '检查是否完成安全影响评估、安全扫描或安全测试，确认无高危漏洞遗留，安全风险与缓解措施清晰可追踪。',
  架构负责人: '检查架构方案、系统边界、上下游接口、容量性能和回退路径，确认变更符合总体技术架构要求。',
  机构负责人: '检查业务影响范围、机构协同事项、窗口安排和上线通知，确认相关行社/部门已完成投产准备。',
  项目负责人: '检查需求、开发、测试、问题闭环和投产申请材料是否齐备，确认交付范围、风险和计划一致。',
  测试负责人: '检查 SIT/UAT/NFT/SEC 测试结论、缺陷遗留、回归范围和测试报告，确认测试准出条件满足。',
...
async function ensureSignoffCheckContent() {
  for (const [code, content] of Object.entries(SIGNOFF_CHECK_CONTENT)) {
    await run(
      'UPDATE role SET signoff_check_content = ?, updated_at = datetime(\'now\',\'localtime\') WHERE code = ? AND is_signoff_role = 1',
      content, code,
    );
  }
}
